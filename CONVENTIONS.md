...
## Audited high-level fast paths

Public low-level helpers must validate inputs by default. A high-level
algorithm may pass `validate_inputs = FALSE` only after it has already checked
the same invariants once for the exact object being passed. This option must
never be the public default. Fast paths must be covered by equivalence tests
against the validated path.

When several parallel stages use the future multisession backend, create one
worker plan before the first stage, reuse it across stages, and restore the
caller's plan when the high-level algorithm exits. Pass required S4 packages
through `future_packages` so methods are registered before workers subset or
transform those objects. Do not recreate a worker pool inside every apply
call.

Avoid retaining large intermediate matrices in a result object when they are
not needed downstream. High-level algorithms that expose a retention choice
must use `retain_intermediates = c("all", "minimal")`; the default remains
`"all"` for compatibility, while `"minimal"` must retain selections, losses,
summaries, timing, options, and diagnostics.
