...
bin_dev <- function(x, y, epsilon = 1e-5, na_rm = TRUE) {
  if (!is.numeric(x) || !is.numeric(y)) {
    stop("x and y must be numeric.", call. = FALSE)
  }
  if (length(x) != length(y)) {
...
bin_dev_mean <- function(x, y, epsilon = 1e-5, na_rm = TRUE) {
  total_loss <- bin_dev(x, y, epsilon = epsilon, na_rm = na_rm)
  if (!na_rm) {
...
auc <- function(A, P, use_cpp = TRUE) {
  group <- as.numeric(A)
  predictions <- as.numeric(P)
  if (!is.numeric(group) || !is.numeric(predictions)) {
    stop("group and predictions must be numeric.", call. = FALSE)
  }
  if (length(group) != length(predictions)) {
  ...
  if (any(!is.finite(group)) || any(!group %in% c(0, 1))) {
  ...
  as.numeric(u_statistic / (n0 * n1))
}

# Convert ROC AUC, where larger is better, into a loss where smaller is better.
auc_as_loss <- function(A, P, use_cpp = TRUE) {
  1 - auc(A = A, P = P, use_cpp = use_cpp)
}
...
procrustes <- function(
  ...
  if (any(!is.finite(X)) || any(!is.finite(X_star))) {
    stop("X and X_star must contain only finite values.", call. = FALSE)
  }
  N <- nrow(X)
  if (N < 1L || N != nrow(X_star)) {
