...
This is the living function reference for `SONNET_unified`. It covers every
...
| `sse()` | Sum of squared errors. | `x`, `y`: equal-length numeric vectors; `na_rm`: remove missing differences. | Numeric scalar. |
...
| `bin_dev()` | Summed binary cross-entropy/negative log-likelihood. The historical name does not imply the extra factor of two. | `x`: binary response; `y`: probabilities; `epsilon`: clipping level; `na_rm`: missing-value removal. | Numeric loss scalar. |
| `bin_dev_mean()` | Mean binary cross-entropy over observed pairs. | Same as `bin_dev()`. | Numeric mean loss. |
| `auc()` | Binary ROC AUC with average ranks for ties. Tries `auc_cpp()` and otherwise uses a base-R rank implementation. | `A`: binary labels; `P`: finite scores; `use_cpp`: accelerator switch. | AUC in `[0,1]`, or `NaN` if only one class occurs. |
...
| `procrustes()` | Orthogonally aligns `X` to `X_star`, optionally translating, dilating, and reporting residual SSE. Pads `X` with zero columns when needed. | `X`, `X_star`: finite matrices with equal rows; `translate`: add translation; `dilate`: estimate scale; `sumsq`: return SSE; `use_cpp`: accelerate translation-only case. | List with `X_new`, `rotation`, and optional `translation`, `scale`, and `sse`. |
...
| Function | Description and use | Arguments | Returns |
|---|---|---|---|
| `generate_adjacency()` | Samples independent Bernoulli edges from `P`. Undirected sampling draws one triangle and constructs a general symmetric matrix with `A + t(A)`; row seeds make results independent of worker count. Full probability validation remains enabled for direct calls. Audited high-level generators disable the repeated full-matrix scan only after constructing and minimally checking their clipped `P`. | `P`; `representation`; `directed`; `self_loops`; `seed`; `ncores`; `validate_inputs = TRUE`: validate dimensions, bounds, finiteness, and symmetry. | Binary adjacency matrix. |
| `generate_er()` | Erdős–Rényi generator parameterized by exactly one of edge probability or expected average degree. | `n`; `p` or `average_degree`; representation, direction, loop, seed, and worker controls. | Adjacency `A`; metadata contains model, `p`, and expected degree. |
...
| `spectral_cluster()` | Spectral clustering for undirected, non-negative networks, or direct clustering of a supplied representation. Its high-level preflight prints an explicit additive formula for decomposition, counted Laplacian operations, dense regularization, and PAM dissimilarities. Laplacian input is always estimated as dense even when stored and processed sparsely. | `A`; optional `U`; `K`; Laplacian, regularization, zero-degree, normalization, decomposition, and clustering controls; `ram_check = TRUE`. Legacy `spectral_options$ram_check` is promoted to this high-level argument. | List with full `g_hat`, clustered `U_hat`, spectral values, zero-degree and retained indices, clustering fit, and selected engines. |
...
| Function | Description and use | Arguments | Returns |
|---|---|---|---|
| `estimate_sbm()` | Estimates SBM block means from a full adjacency or a common rectangular NCV layout. Rectangular denominators count the dyads actually present, genuine self-pairs are identified from original node indices, and undirected sufficient statistics pool both available orientations. A full matrix supplied with folds is reduced to its non-fold rows. Blocks with no observable dyads return `NA`. | `A`: full or rectangular finite numeric adjacency; `g`: positive integer labels for all nodes; `K`: total communities; `fold_nodes`: optional held-out node indices; `directed`: retain directional block means instead of pooling transposes; `self_loops`: include matched-node diagonal dyads. | Named `K` by `K` block-mean matrix `B_hat`. |
| `estimate_dcbm()` | Unified plug-in/spectral estimator of DCBM block and degree parameters for full networks and four common NCV matrix layouts. It corrects rectangular node-index mappings. The spectral method uses `eig_decomp()` for a full square network and `singular_decomp()` for rectangular data; supplied norms avoid recomputation. | `A`: full/NCV adjacency; `g`, `K`: full node labels and community count; `method`: `"plugin"` or `"spectral"`; `fold_nodes`: optional held-out indices; `row_norm`: optional full vector, compatible column vector, or list with layout-specific `row`/`col` vectors; `psi_omit`: omit leading full-network estimates; `stabilizer`: plug-in block pseudocount; `spectral_engine`, `spectral_options`: decomposition controls. | List with named block matrix `B_hat` and node degree parameters `psi_hat` (all retained nodes in the full case or fold nodes in NCV). |
...
| Function | Description and use | Arguments | Returns |
|---|---|---|---|
| `netcrop_blockmodel()` | Selects SBM/DCBM and `K` by the original three-stage overlapping-subnetwork flow: one shared rank-`K_max` representation per subgraph, candidate-specific clustering through `spectral_cluster(U = ...)`, parameter estimation, and unchanged pairwise held-out loss reduction. Supports either/both models and arbitrary positive integer candidate sets, including singleton sets. Asymmetric input is replaced by the binarized undirected union. Decomposition, parameter estimation, prediction, and held-out responses consistently use this same binary adjacency scale. Per-stage task seeds are invariant to worker count; smart worker counts are `min(loop_length, ncores)`. Standard loss functions travel with a compact self-contained helper environment for Windows/future workers. The preflight prints stage-specific sequential and parallel RAM factors. Missing overlap communities are reported without changing matching. A strict sparse input remains sparse through adjacency-derived preprocessing when `regularize_tau = 0`; symmetric sparse storage is converted to general sparse storage for RSpectra compatibility, normalized scaling uses sparse diagonal matrices, and dense decomposition fallback is forbidden. Positive scalar regularization explicitly permits densification. Resolved clustering options are filtered to arguments accepted by the selected k-means, CLARA, or PAM engine. Sparse finiteness is checked once through stored values, and repeated low-level decomposition validation is disabled only after this high-level validation succeeds. | `A`; `K_candidates`; `num_subnetworks`; `overlap_size`; `nrep = 1`; helper names in `losses`; `model_candidates`: SBM and/or DCBM; nested `sbm_est_options` and `dcbm_est_options`, each with `spectral_cluster` and `estimate`; `matching_method`: greedy, Hungarian, or brute force; `confirm_large`; `ncores`; `seed`; `verbose`; `force_windows`; `ram_check`. CLARA defaults to `samples = 50`, Euclidean/non-normalized for SBM and Manhattan/row-normalized for DCBM. Shared decomposition settings must agree when both models are fitted. | Classed `netcrop_blockmodel` object containing detailed and aggregated CV losses, per-repetition and overall selections, candidate table, partitions, raw and estimated intermediates, averaged block estimates, matching diagnostics, resolved options, worker counts, timings, and RAM diagnostics. |
...
| Function | Description and use | Arguments | Returns |
|---|---|---|---|
| `netcrop_rdpg()` | Selects a symmetric-RDPG latent dimension using the original NETCROP flow: magnitude-ordered subnetwork eigendecompositions of unscaled `A`, candidate embeddings formed with square roots of absolute eigenvalues, overlap Procrustes alignment, and pairwise loss evaluation against binary held-out blocks of `A`. Predictions are reconstructed on that same scale and probability-clipped without an additional `rho_hat` multiplier; `rho_hat` remains an output diagnostic. Supports arbitrary positive candidate sets, including singletons. Remainders augment repetition-specific overlaps. Standard losses are bundled with dependencies for multisession workers; strict worker errors identify the originating task. The high-level RAM preflight reports decomposition, embedding, rotation, and held-out product terms using actual smart worker counts. Repeated low-level matrix validation is disabled only after the high-level validation succeeds. | `A`; `d_candidates`; `num_subnetworks`; `overlap_size`; `nrep = 1`; `losses`; `eig_options`; `ncores`; `seed`; `verbose`; `force_windows`; `ram_check`. Default decomposition is forced RSpectra and magnitude ordered. | Classed `netcrop_rdpg` object containing detailed and aggregated CV losses, per-repetition and overall dimensions, partitions, task grids, raw decompositions, candidate and aligned embeddings, negative-eigenvalue diagnostics, resolved options, worker counts, timings, and RAM diagnostics. |
