...
netcrop_blockmodel <- function(
  ...
  A_was_binary <- all(as.numeric(A) %in% c(0, 1))
  A_binary <- A != 0
  if (!A_was_symmetric) {
    warning(
  ...
  rownames(A) <- rownames(A_binary)
  colnames(A) <- colnames(A_binary)
  ...
  spectral_defaults <- list(
    ...
    cluster_options = list(samples = 50L)
  )
  sbm_defaults <- list(
  ...
  estimation_task_count <- raw_task_count * length(K_candidates)
  ...
  if (ram_check) {
    ...
    retained_storage <-
      ...
      estimation_task_count * length(model_candidates) * (
        ...
        for (candidate_id in seq_along(K_candidates)) {
          ...
          if ("DCBM" %in% model_candidates) {
            labels_by_model$DCBM[[candidate_id]] <-
              cluster_from_U(U_K, K, dcbm_options$spectral_cluster)$g_hat
          }
        }
        list(
  ...
  estimation_grid <- expand.grid(
    ...
    raw_task_id = seq_len(raw_task_count),
    KEEP.OUT.ATTRS = FALSE
  )
  estimation_grid <- estimation_grid[
    order(estimation_grid$raw_task_id, estimation_grid$candidate_id),
  ]
  ...
  if (verbose) {
    message(
      "Stage 2/3: estimating ", estimation_task_count,
      " subnetwork/candidate combinations with ", estimation_ncores,
      " worker(s)."
    )
  }
  estimation_time <- system.time({
    estimation_output <- uni_mclapply(
      ...
      function(task_id) {
        ...
        set_task_seed(2L, task_id)
        raw_task_id <- estimation_grid$raw_task_id[task_id]
        candidate_id <- estimation_grid$candidate_id[task_id]
        ...
        standard_task_id <- which(
          task_grid$repetition == repetition & task_grid$split == 1L
        )
        ...
        for (model_id in seq_along(model_candidates)) {
          ...
          if (split_id == 1L || K == 1L) {
            ...
            matched <- match_labels(
              match_this = labels[seq_len(overlap_count)],
              standard = standard[seq_len(overlap_count)],
              K = K
            )
            matched$matched_labels <- unname(matched$mapping[labels])
          }
          matched_labels <- as.integer(matched$matched_labels)
          missing_match <- setdiff(
            seq_len(K),
            unique(labels[seq_len(overlap_count)])
          )
          missing_standard <- setdiff(
            seq_len(K),
            unique(standard[seq_len(overlap_count)])
          )
          diagnostics[[model_id]] <- data.frame(
          ...
          if (model == "SBM") {
            protected <- c("A", "g", "K", "directed", "self_loops")
            conflicts <- intersect(names(sbm_options$estimate), protected)
            if (length(conflicts) > 0L) {
              stop(
                "sbm_est_options$estimate cannot override ",
                paste(conflicts, collapse = ", "), ".",
                call. = FALSE
              )
            }
            ...
            protected <- c("A", "g", "K", "row_norm", "psi_omit")
            conflicts <- intersect(names(dcbm_options$estimate), protected)
            if (length(conflicts) > 0L) {
              stop(
                "dcbm_est_options$estimate cannot override ",
                paste(conflicts, collapse = ", "), ".",
                call. = FALSE
              )
            }
            estimate_options <- dcbm_options$estimate
            estimation_method <- if (is.null(estimate_options$method)) {
              "plugin"
            } else {
              match.arg(estimate_options$method, c("plugin", "spectral"))
            }
            row_norm <- if (estimation_method == "spectral") {
        ...
        list(models = model_results, diagnostics = diagnostics)
      },
  ...
  estimate_lookup <- function(repetition, split_id, candidate_id, model) {
    raw_task_id <- which(
      task_grid$repetition == repetition & task_grid$split == split_id
    )
    task_id <- which(
      estimation_grid$raw_task_id == raw_task_id &
        estimation_grid$candidate_id == candidate_id
    )
    estimation_output[[task_id]]$models[[model]]
  }
  ...
  loss_time <- system.time({
    loss_output <- uni_mclapply(
      ...
      function(task_id) {
        ...
        records <- vector(
          "list",
          length(model_candidates) * length(losses)
        )
        record_id <- 1L
        for (model in model_candidates) {
          left <- estimate_lookup(
            repetition,
            split_left,
            candidate_id,
            model
          )
          right <- estimate_lookup(
            repetition,
            split_right,
            candidate_id,
            model
          ...
          for (loss_name in losses) {
            loss_value <- loss_functions[[loss_name]](
              as.numeric(A_test),
              P_hat
            ) / pair_count
  ...
  matching_diagnostics <- do.call(rbind, unlist(lapply(
    estimation_output,
    `[[`,
    "diagnostics"
  ), recursive = FALSE))
...
summary.netcrop_blockmodel <- function(object, ...) {
  underidentified <- object$matching_diagnostics$underidentified
  result <- list(
    call = object$call,
    ...
    underidentified_matches = sum(underidentified),
    total_matches = length(underidentified),
