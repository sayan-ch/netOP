...
| `oracle_plotter()` | Computes accuracy as one minus the validated label-matching mismatch rate and plots mean accuracy. A single network produces points without error bars; multiple networks add horizontal plus/minus-one-SD bars. At least one NETCROP or DKEST outcome supplies the candidate grid. Mismatched grids warn and use their floating-point-tolerant intersection; an empty intersection is an error. Engine-specific oracle accuracy is compared with every available tuner selection: NETCROP's first repetition and optionally its repetition mean and mode for each chosen loss, plus DKEST's direct `tau_hat`. Off-grid selected values trigger an extra fit. Separate SONNET and spectral-clustering `tau = 0` baselines are included by default without expanding the oracle grid. `engines` independently selects SONNET, spectral clustering, or both, so every supplied tuner's tau is evaluated under every requested engine regardless of provenance. Outcome-derived and explicitly overridden clustering engines receive compatible backend-specific defaults, with CLARA as the current default. | `A` and required `g_true`: one matched matrix/vector pair or same-length lists with shared dimensions; optional `K`; at least one of `netcrop_outcomes` or `dkest_outcomes`, where scalar inputs accept one classed output and list inputs require matching outcome lists; `include_netcrop_mean = TRUE`; `include_netcrop_mode = TRUE`; optional `losses`; `engines = c("sonnet", "spectral_cluster")`; `include_sonnet_tau_zero = TRUE`; `include_spectral_tau_zero = TRUE`; validated `matching_method`; brute-force confirmation; named fitter option lists; standard worker, seed, verbosity, cross-platform, and RAM controls. | Invisibly returns the rendered `ggplot`, faceted by requested engine. Raw accuracies, aggregated plotting data, the common tuner-derived candidate grid, effective fit metadata, and diagnostics are attached as `accuracy_data`, `plot_data`, `metadata`, and `diagnostics` attributes. |
